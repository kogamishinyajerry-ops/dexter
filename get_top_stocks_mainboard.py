...
def calculate_quant_score(financial_data, quote_data):
    ...
    if 0 < quote_data['pe'] < 15:
        score += 15
        reasons.append(f"✓ 估值偏低(PE={quote_data['pe']:.2f})，安全边际高")
    elif 15 <= quote_data['pe'] < 30:
        score += 12
        reasons.append(f"✓ 估值合理(PE={quote_data['pe']:.2f})")
    elif 30 <= quote_data['pe'] < 50:
        score += 8
        reasons.append(f"✓ 估值适中(PE={quote_data['pe']:.2f})")
    elif quote_data['pe'] > 50 and financial_data['revenue_growth'] > 30:
        score += 5
        reasons.append(f"✓ 高成长高估值(PE={quote_data['pe']:.2f})")
...
def generate_report(results):
    ...
    for idx, (_, row) in enumerate(top15.iterrows()):
        ...
        risks = []
        if row['pe'] > 60:
            risks.append(f"估值偏高(PE={row['pe']:.2f})，存在回调风险")
        if row['turnover'] > 15:
    ...
    # 保存详细数据
    output_file = f"/workspace/dexter/top_stocks_mainboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
